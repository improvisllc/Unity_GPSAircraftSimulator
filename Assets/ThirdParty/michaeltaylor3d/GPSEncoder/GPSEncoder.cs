...
using UnityEngine;
// OPERA GPS LAT: 40.184996 LON: 44.514981
public sealed class GPSEncoder {
	...
	private Vector3 ConvertGPStoUCS(Vector2 gps)  
	{
        ...
        //Debug.Log("_LatOrigin1: " + _LatOrigin);
	...
	private Vector2 ConvertUCStoGPS(Vector3 position)
	{
        ...
        /*Debug.Log("_LatOrigin2: " + _LatOrigin);
        Debug.Log("position.z: " + position.z);
        Debug.Log("position.x: " + position.x);

        Debug.Log("metersPerLat: " + metersPerLat);
        Debug.Log("metersPerLon: " + metersPerLon);*/

        //float XXX = _LatOrigin + (position.z / metersPerLat);
        

        float geolocationX = _LatOrigin + (position.z / metersPerLat); //Calc current lat
        float geolocationY = _LonOrigin + (position.x / metersPerLon); //Calc current lon

		return new Vector2((float)geolocationX, (float)geolocationY);
	}
	#endregion
}
